...
const Order = () => {
  ...
  return (
    <div>
        <div className={`${isHiddenLogin && isHiddenAlert && isHiddenAlertDeleteCart && isHiddenAlertChangeCart && isHiddenMethodReceive && isHiddenComboDetail?'':'pointer-events-none brightness-50 h-screen  overflow-y-scroll'} bg-white flex z-20`}>
            ...
            <div className='w-1/4 fixed right-0 h-screen bg-white border-l shadow-sm'>
                ...
                <div className='bg-[#f5f7f9] h-[106px] p-[5px] '>
                    ...
                    <div className='flex justify-between'>
                        <div className='flex justify-between w-[50%] cursor-pointer' onClick={()=>handleShowInformation("Điều khoản và điều kiện áp dụng Giảm giá Thành Viên được quy định theo Chương Trình Khách Hàng thân thiết của Pizzeria Việt Nam.")}>
                    ...
                    <div className='flex justify-between'>
                        <div className='flex justify-between w-[50%] cursor-pointer' onClick={()=>handleShowInformation("Chi tiết về điều khoản và điều kiện áp dụng được quy định theo các chương trình khuyến mại của Pizzeria tại từng thời điểm cụ thể.")}>
                    ...
                    <div className='flex justify-between'>
                        <div className='flex justify-between w-[50%] cursor-pointer' onClick={()=>handleShowInformation("Phí giao hàng 22.000đ cho các sản phẩm combo hoặc chương trình khuyến mãi.")}>
                        ...
                        <div>{formatCurrency((order.pizzaInOrders.length+ order.foodInOrders.length + order.comboInOrders.length)===0 ? 0 : methodReceive? 22000:0)}</div>
                    </div>
                </div>
                <div className='w-full h-[56.5px] p-[10px]'>
                    <button className='bg-[#0A8020] text-white rounded-[4px] h-full w-full text-sm flex items-center text-end hover:shadow-xl'onClick={handleToPayment}>
                        ...
                        <div className='h-4 w-[50%] pr-4 '>{formatCurrency((order.pizzaInOrders.length+ order.foodInOrders.length + order.comboInOrders.length)===0?0:order.total+ (methodReceive? 22000:0))}</div>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div className={`absolute top-[30%] left-[35%] z-50 `}>
            {
                isHiddenMethodReceive?'':<MethodReceive showClose = "true" methodReceive={methodReceive} setMethodReceive={setMethodReceive}/>    
            }
        </div>
